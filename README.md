...
* 说明
  ...
  - `views.py`
    ...
    - `login()`
      ...
      用户在前端页面输入的数据`POST`方式发送到服务器，服务器通过此函数接收处理发来的请求，`is_valid()`验证数据，验证后会从`cleaned_data`得到form中的值。数据会和数据库中相匹配，在输入的数据有错误时会有提示`message`。用`cookies`技术进行权限的校验，登陆验证成功则会跳转到`/monitor/`。
